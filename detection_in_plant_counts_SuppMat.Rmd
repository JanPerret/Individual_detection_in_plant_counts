---
title: "Supplementary Material for 'Plants stand still but hide: imperfect and heterogeneous detection is the
  rule when counting plants'"
author: "Jan Perret ([orcid](https://orcid.org/0000-0002-1912-6995)), Aurélien Besnard ([orcid](https://orcid.org/0000-0002-2289-9761)), Anne Charpentier ([orcid](https://orcid.org/0000-0002-8588-6001)), Guillaume Papuga ([orcid](https://orcid.org/0000-0002-7803-2219))"
...
#   ylab("Proportion of detected individuals") +
...
df_session_recap_for_print <- subset(df_session_recap_for_print, df_session_recap_for_print$Date != "31/07/2020")

# # print the session recap table
# df_session_recap_for_print %>%
#   kbl(align = c('l', 'l', 'l', 'l', 'l', 'c')) %>%
#   kable_styling()
...
vect_sessions_to_remove <- c("31/07/2020", "16/10/2020", "20/10/2020", "28/10/2020", "23/04/2021", "19/05/2021", "26/05/2021", "27/05/2021")
...
**Figure S1.1:** Map of the 24 study sites. Some sites were a few kilometers apart, so the corresponding points overlap.  <br>
...
For each field session, we visually estimated in which range the mean height of the individuals of the target species was, and recorded whether the majority of individuals were flowering or not. Inconspicuous flowers (i.e. without petals or without vivid colors) were not taken into account. We then created the species conspicuousness variable with the following criteria : <br>

1  ->  Individuals [0 cm; 5 cm] tall, without colorful flowers. <br>
2  ->  Individuals [5 cm ; 10 cm] tall, or [0 cm ; 5 cm] with colorful flowers. <br>
3  ->  Individuals [10 cm ; 20 cm] tall, without colorful flowers. <br>
4  ->  Individuals [20 cm : +Inf[ tall, or [10 cm ; 20 cm] with colorful flowers. <br>

**Table S2.1:** Values of the species conspicuousness variable for the 30 sampled species.  <br>

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# table with the species_conspicuousness values for all the sampled species

# # print the table
# kable(df_species_conspicuousness[order(df_species_conspicuousness$species_conspicuousness, df_species_conspicuousness$species_name), ],
#       align = c('l', 'l', 'c')) %>%
...
In order to obtain a synthetic measure of vegetation closure, we performed a PCA on the two variables (i.e. vegetation cover rate and vegetation height), and kept the quadrat coordinates on the first principal component.  <br>
...
The first principal component explained `r pca_explained_variance_1`% of the total variance, and the second explained `r pca_explained_variance_2`% of the variance. The Figure S2.2 shows the value associated to each quadrat on the first principal component, i.e. our new composite variable of habitat closure, as a function of the cover rate and height of the vegetation.   <br>
...
**Figure S2.2:** Value of the 'habitat closure' variable as a function of vegetation height and cover.  <br>

<br>

## Evaluation of the experience in botany of the observers

Before we explained the experiment to the observers, they were asked to fill in a form including a rating of their level of experience in botany on a scale of 1 to 10. In order to homogenise the ratings between observers of all field sessions, a brief description was provided for every second level of this experience scale. We subsequently divided the experience level by two to obtain a scale with 5 classes.  <br>

The form was as follows :  <br>

**How do you evaluate your experience in botany? (circle your level)**  <br>
The number of known species is given as a guide, if the description of a level fits you but you can easily recognise more species than the indicated range, you can upgrade to the higher category if you feel that it better fits your experience level. A 'known species' means that you can identify it with certainty without using an identification guide, or simply by quickly checking a single criterion.  <br>


1  <br>
2 : You have already practised botany but rarely do so, or you have not practised for a long time. (Less than 50 known species)  <br>
3  <br>
4 : You practice botany occasionally, or you started less than 3 years ago. (50-300 known species)  <br>
5  <br>
6 : You practice botany regularly and for more than 3 years. (300-1000 known species)  <br>
7  <br>
8 : You have been practising botany intensively for more than 5 years, and are developing an expertise. (1000-2000 known species)  <br>
9  <br>
...
**Figure S3.1:** True number of individuals per quadrat and field session, before the four density outliers were removed. Each point represents a quadrat, and the dotted red line shows the threshold of 300 individuals above which we removed the quadrats from the dataset to avoid the risk of leverage effect. <br>
...
**Figure S3.3:** Mean proportion of detected individuals per observer after the excess detections were removed. Each point represents the mean detection rate of an observer during a field session. <br>
...
A total of `r nrow(df_3_methods_0)` observations composed the dataset before removing the `r n_empty_quadrats` quadrats that didn't contain any individual of the target species. `r n_obs_empty_quadrats` observations were made on these empty quadrats, only `r n_obs_excess_empty_quadrats` of which with excess detections. We believe that this shows that identification errors were extremely rare during the experiment, and that the excess detections were mainly due to errors in delimiting individuals.  <br>

The description below is made on the dataset after removing the quadrats that didn't contain any individual of the target species. After this filtering, the dataset contained `r nrow(df_3_methods_excess)` observations, of which `r n_obs_excess_without_empty_quadrats` excess detections.  <br>
...
The observations with excess detections constituted less than 6.8% of the observations per field session, except for the session on Erigeron annuus (ERIANN; 26.7% observations over 100% detection). For this field session the detection of individuals was very easy because the individuals were in full flowering and taller than the surrounding vegetation, which resulted in a high detection rate. Furthermore, many individuals had multiple stems and the ramification was close to the ground. It was possible to delimit the individuals unambiguously by looking closely at the ramification, but the observers did not pay close attention. The combination of an easy detection and this particular morphology of individuals led to these excess detections.  <br>

<br>

**Table S4.2:** Distribution of the observations with excess detections per level of experience in botany.  <br>
...
**Figure S4.1:** Difference between the counts made by the observers and the true number of individuals present in the quadrats. The lower part of the graph is truncated, but not the upper part, so that all excess detections are represented. The majority of the excess detections occurred on quadrats containing few individuals, and were below 110% of the true number of individuals.  <br>



<br>

# Appendix 5: Model comparison table

The model comparison is presented in the file "df_model_selection.csv" joined separately from the present document.  <br>

<br>

# Appendix 6: Coefficients of the selected model

All predictors were standardised before the model fitting. Here is the model fitting function using the R package lme4 :  <br>
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# print the table
# kable(df_mod_final_s_betas_random_effects,
#       align = c('l', 'r', 'r'),
#       digits = 4) %>%
#   kable_styling(full_width = TRUE)
...
**Figure S6.1:** Predicted detection probability depending on the level of experience in botany of the observer under easy detection conditions (species conspicuousness = 4; habitat closure = -1.5), average conditions (species conspicuousness = 2.5; habitat closure = 0.5) and hard conditions (species conspicuousness = 1; habitat closure = 2.5). Only the values under average conditions are used in the main text. <br>
...
The three figures below are the alternative versions of Figs 2-4 from the main text, made by fitting the model to the dataset in which the excess detections were set to 100% detection rate instead of removing them from the dataset. The differences are imperceptible, which indicates that the excess detections don't play any role in the results of our analyses.   <br>
...
df_mod_final_excess_to_1_s_betas$term <- gsub(pattern = "exp_bota", replacement = "Experience botany", x = df_mod_final_excess_to_1_s_betas$term, fixed = TRUE)
...
**Figure S7.1:** Coefficients of standardized variables in the selected model. Dots represent estimated coefficients, and lines represent 95% confidence intervals. Positive coefficients (blue) indicate that when the variable increases, the proportion of detected individuals increases, while negative coefficients (red) indicate the opposite. Coefficients with a non-significant effect are shown in gray. For the variable "counting method", the reference level is the estimate of the number of individuals in 30 seconds. The coefficients presented for the two other methods indicate the difference in mean detection rate obtained with those methods compared to the reference level.  <br>
...
**Figure S7.2:** Predicted proportion of detected individuals depending on species conspicuousness, habitat closure, counting time and the counting method used. The other variables included in the model are held constant at the following levels : exp_bota = 2.5, count_TRUE = 100, quadrat_order = 5. Lines represent the mean proportion of detected individuals, and the shading represent 95% confidence intervals.  <br>
...
The three figures below are the alternative versions of Figs 2-4 from the main text, made by fitting the model to a version of the dataset where only the observers with a high level of experience in botany were kept (i.e. levels of experience of 4 and 5 out of 5). This reduced the number of observations in the dataset from `r nrow(df_3_methods)` to `r nrow(df_3_methods_xp)`. This induced substantial differences in the parameter estimates and in the model's predictions, but the general pattern of the predictions is maintained. It shows that our results are not due to an excess of observers with little or no field experience in our sample of observers.  <br>
...
We warmly thank the following 167 volunteers who participated in the experiment, some of them for multiple times, including during the preliminary tests:  <br>
